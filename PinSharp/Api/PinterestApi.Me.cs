...
namespace PinSharp.Api
{
    public partial class PinterestApi : IMeApi
    {
        ...
        {
            return await GetAsync<UserDetails>("me", UserFields);
        }

        public async Task<IEnumerable<UserBoard>> GetBoardsAsync()
        {
            var fields = BoardFields.Where(x => !x.StartsWith("creator"));
            return await GetAsync<IEnumerable<UserBoard>>("me/boards", fields);
        }
        ...
        async Task<PagedResponse<UserPin>> IMeApi.GetPinsAsync(string cursor, int limit)
        {
            ...
            var response = await GetPagedAsync<UserPin>("me/pins", fields, cursor, limit);
        ...
        {
            var fields = PinFields.Where(x => !x.StartsWith("creator"));
            var response = await GetPagedAsync<UserPin>("me/likes", fields, cursor, limit);
            return new PagedResponse<UserPin>(response.Data, response.Page?.Cursor);
        }

        public async Task<PagedResponse<User>> GetFollowersAsync(string cursor, int limit)
        {
            var response = await GetPagedAsync<User>("me/followers", cursor, limit);
        ...
        {
            var response = await GetPagedAsync<Board>("me/boards/suggested", BoardFields, cursor, limit);
        ...
        {
            var response = await GetPagedAsync<Board>("me/following/boards", BoardFields, cursor, limit);
            return new PagedResponse<Board>(response.Data, response.Page?.Cursor);
        }

        public async Task<PagedResponse<Interest>> GetFollowingInterestsAsync(string cursor, int limit)
        {
            var response = await GetPagedAsync<Interest>("me/following/interests", cursor, limit);
        ...
        {
            var response = await GetPagedAsync<User>("me/following/users", cursor, limit);
            return new PagedResponse<User>(response.Data, response.Page?.Cursor);
        }
        ...
        {
            var fields = BoardFields.Where(x => !x.StartsWith("creator"));
            var response = await GetPagedAsync<UserBoard>($"me/search/boards/?query={query}", fields, cursor, limit);
        ...
        public async Task<PagedResponse<UserPin>> SearchPinsAsync(string query, string cursor, int limit)
        {
            ...
            var response = await GetPagedAsync<UserPin>($"me/search/pins?query={query}", fields, cursor, limit);
            return new PagedResponse<UserPin>(response.Data, response.Page?.Cursor);
        }

        public async Task FollowBoardAsync(string board)
        {
            await PostAsync("me/following/boards", new {board});
        }
        ...
        {
            await PostAsync("me/following/users", new {user});
        }
